...
from ibl_tools.util_files import get_dir_files, search_str_files, extract_group_to_individual_class, \
...
class ExperimentNames(object):
    def __init__(self):
        self.DIR_INPUT = "/run/user/1000/gvfs/smb-share:server=locker-smb.engram.rc.zi.columbia.edu,share=paninski-locker,user=ADCU%5Cekb2154/data/ibl/dlc-networks"
    ...
    def get_raw_files(self):
        ...
        dlc_filenames = list(filter(lambda x: re.search(DLC_EXTENSION, x),
                                    FNAMES_RAW))
        dlcnames, dlcextensions = zip(*[os.path.splitext(x) for x in dlc_filenames])

        video_filenames = list(filter(lambda x: re.search(VIDEO_EXTENSION, x),
                                      NAMES_RAW))
        vnames, vextensions = zip(*[os.path.splitext(x) 
                                    for x in video_filenames])

        # assert we will find traces
        sorted_dlc_names = []
        for video_name in vnames:
            dlc_name_match = list(filter(lambda x: re.search(video_name, x),
                                         dlcnames))
...
class ExperimentFiles(object):
    ...
    def __init__(self, experiment_name, subject_id):
        ...
        self.DIR_OUTPUT_PREPROCESS = os.path.join(self.DIR_OUTPUT,
                                                  self.DIR_PREPROCESS,
                                                  self.DIR_BODY_PART)

        # Set directory for features from traces
        self.DIR_OUTPUT_FEATURE = os.path.join(self.DIR_OUTPUT,
                                               self.DIR_FEATURE,
                                               self.DIR_BODY_PART)
    ...
    def set_out_fnames(self):
        fname = os.path.join(self.DIR_OUTPUT_PREPROCESS, 
                             self.fname_preprocessed)
        self.FILE_PREPROCESS = fname
        
        fname = os.path.join(self.DIR_OUTPUT_FEATURE, 
                             self.fname_features)
    ...
    def set_raw_fnames(self):
        ...
        raw_filenames = search_str_files(DIR_INPUT_DLC_FILES,
                                         str_match=self.EXPERIMENT_NAME)

        # Raw video filename
        video_fname = search_str_files(raw_filenames,
                                       str_match=self.VIDEO_EXTENSION)
        assert len(video_fname) == 1

        self.VIDEO_FNAME = os.path.join(DIR_INPUT_DLC, video_fname[0])

        # Raw DLC trace filename
        dlc_fname = search_str_files(raw_filenames,
                                     str_match=self.DLC_FNAME_EXTENSION)
    ...
    def load_trace_features(self, as_mean=True):
        ...
        if os.path.isfile(self.FILE_FEATURES): 
            (train_, val_, test_) = np.load(self.FILE_FEATURES,
                                            allow_pickle=True)
        else:
            return (0, 0, 0)
